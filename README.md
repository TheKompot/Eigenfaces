...
## Overview
This project is focused on using Principal Component Analysis (PCA) on photos. We are implementing PCA algorithm using `numpy` library in Python and comparing it with  build-in [PCA](https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.PCA.html) algorithm in `scikit.learn`. Using PCA algorithm and two different clustering methods ([K-means](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html) and [Gaussian Mixture algorithm](https://scikit-learn.org/stable/modules/generated/sklearn.mixture.GaussianMixture.html) from `scikit.learn`) we are trying to find groups of teachers at our faculty who look alike. We are also curious about which of our teachers look alike famous mathematicians (Gauss, Neumann, Turing, Einstein). 
